...
class Dialog_error_proxy(QDialog):
    root_project_dir = '..'
    ...
    def __init__(self,ip: str,port: str,login: str,password: str):
        ...
        self.label_info = DraggableLabel("  Введите корректные данные!",self)
...
"}")
    ...
    def show_info(self):
        error_info = Dialog_info('Внимание!', 'При попытке подключиться к прокси\nпроизошла ошибка входа','notification.mp3')  # Создаем экземпляр
    ...
    def _enter_proxy(self):
        ...
        if ip == '' or port == '':
            error_info = Dialog_info('Внимание!', 'Не все данные заполнены','notification.mp3')  # Создаем экземпляр
            error_info.exec_()  # Открываем
            return
    ...
    def retranslateUi(self):
        ...
        self.pushButton_not_use_proxy.setText(_translate("Dialog", "Не использовать прокси"))
...
if __name__ == "__main__":
    ...
    ui = Dialog_error_proxy('83.217.5.96','11225','FvngSoSAQj','LNyr1N65pR')
    ui.show()
    sys.exit(app.exec_())
